export const calculateSB = (accel_x, accel_y, accel_z, age, barHeight) => {
  ...
  if (result.enmoValue <= 0) {
    result.enmoValue = 0;
    result.enmoProgressHeight = barHeight * 0.01; // max height of the bar
  } else if (result.enmoValue <= result.target[0]) {
    result.enmoProgressHeight =
      (result.enmoValue / result.target[0]) * barHeight;
  } else if (
    result.enmoValue >= result.target[0] &&
    result.enmoValue <= result.target[1]
  ) {
    result.enmoProgressHeight =
      ((result.enmoValue - result.target[0]) /
        (result.target[1] - result.target[0])) *
      barHeight;
