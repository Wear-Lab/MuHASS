...
import { calculateSPO2, convertTemp } from "../MonitorCalculations";
...
const RawData = () => {
  ...
  const environment =
    dataFile.environment && dataFile.environment[0]?.split(" ").map(parseFloat);
  ...
  const magnetic =
    dataFile.magnetic && dataFile.magnetic[0]?.split(" ").map(parseFloat);
  const [mag_x, mag_y, mag_z] = magnetic || [null, null, null];
  const acceleration =
    dataFile.acceleration &&
    dataFile.acceleration[0]?.split(" ").map(parseFloat);
  const [accel_x, accel_y, accel_z] = acceleration || [null, null, null];
  const gyroscope =
    dataFile.gyroscope && dataFile.gyroscope[0]?.split(" ").map(parseFloat);
